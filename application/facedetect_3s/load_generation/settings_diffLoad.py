...
simSecDur = [300 for _ in range(16)]
...
def interarrivalTimes(simSecDur):
    ...
    x = [1/8, 1/16, 1/24, 1/32]
    ratios = [x[i % 4] for i in range(16)]
    def timefunc(t):
        i = 0
...
opt_fetch.url = "http://10.0.1.153:31111/fetch/"
opt_fetch.setID("st")
...
def interarrivalTimes_2(simSecDur):
    simIntEnd = np.cumsum(simSecDur)
    x = [1/0.2, 1/0.8, 1/1.4, 1/2]
    [x[i // 4] for i in range(16)]
    def timefunc(t):
        i = 0
        for ssd in simIntEnd[:-1]:
            if t < ssd:
