...
# Point towards folder gathered from scaling experiment
datafolder = ""
...
function getPredCoefficents(idx, class_shifts, queue_shifts, t)
    ph_pred = ph_vec[:, idx]
    p_pred = p_opt[:, idx]

    for i in keys(class_shifts)
        if queue_type[classes[i][1]] == "d"
            ph_pred[class_shifts[i]] = createErlangPH(0.025, phases["d"])
        else
            ph_pred[class_shifts[i]] = ph_pred[i]
        end
    end

    for i in keys(queue_shifts)
        p_pred[queue_shifts[i]] = p_pred[i]
    end
...
# p[1] - prediction fit index
# p[2] - what classes to shift
# p[3] - what queues to shift
function dx_fluid_pred!(dx, x, p, t)

    # 2 -> 11, 13
    # 4 -> 11, 15

    ph_pred, p_pred = getPredCoefficents(p[1], p[2], p[3], t)
    ...
    f2 = ph_pred[12].t' * θ[N .== 12]
    f4 = ph_pred[14].t' * θ[N .== 14]

    P_pred = P[:,:,si_idx(t)]
    P_pred[11, 13] = f2+f4 == 0 ? 0.5 : f2/(f2+f4)
    P_pred[11, 15] = f2+f4 == 0 ? 0.5 : f4/(f2+f4)

    W = Ψ + B*P_pred*A'
    dx .= W' * θ + sum(A*λ[:,si_idx(t)], dims=2)[:]
end

pred_fit_idxs = [1, 6]

queue_shifts = [Dict{Int, Int}(2 => 3), Dict{Int, Int}()]
class_shifts = [findScalingClassShifts(queues[2], queues[3], classes, queue_type), Dict{Int, Int}()]
...
prob_pred = [ODEProblem(dx_fluid_pred!, x0, tspan, (idx, class_shifts[i], queue_shifts[i])) 
    for (i, idx) in enumerate(pred_fit_idxs)]
...
for (i, (q, _, _)) in enumerate(classes)
    for j = 1:si
        ...
        if length(tw_class_split_mc[i, j]) == 0
            continue
        end
        ...
        for (idx, pred_idx) in enumerate(pred_fit_idxs)
            ph_pred, p_pred = getPredCoefficents(pred_idx, class_shifts[idx], 
                queue_shifts[idx], (simSecEnd - simSecDur)[j])
...
open("paper_data/extracting_experiment_scale.csv", "w") do f
    writedlm(f, headers_1, ",")
    writedlm(f, hcat(1:11, (1:11) .- 0.2, (1:11) .+ 0.2,
        util[isService, :]',
